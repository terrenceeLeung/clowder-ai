...
export class GovernanceStateMachine {
  ...
  transition(anchor: string, to: GovernanceStatus): void {
    ...
    this.db.prepare('UPDATE evidence_docs SET governance_status = ? WHERE anchor = ?').run(to, anchor);
  }

  getStatus(anchor: string): GovernanceStatus | null {
    const row = this.db.prepare('SELECT governance_status FROM evidence_docs WHERE anchor = ?').get(anchor) as
  ...
  listByStatus(status: GovernanceStatus, packId?: string): string[] {
    if (packId) {
      return (
        this.db
          .prepare('SELECT anchor FROM evidence_docs WHERE governance_status = ? AND pack_id = ?')
          .all(status, packId) as Array<{ anchor: string }>
      ).map((r) => r.anchor);
    }
    return (
      this.db.prepare('SELECT anchor FROM evidence_docs WHERE governance_status = ?').all(status) as Array<{
        anchor: string;
      }>
    ).map((r) => r.anchor);
  }
