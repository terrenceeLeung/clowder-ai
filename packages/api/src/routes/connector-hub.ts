...
export const CONNECTOR_PLATFORMS: PlatformDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        envName: 'XIAOYI_API_ID',
        label: 'Push API ID',
        sensitive: false,
        optional: true,
        help: '创建 Webhook 后生成；仅异步 Push 通知需要',
      },
    ],
    ...
    steps: [
      ...
      { text: '如需异步 Push 通知，创建 Webhook 获取 API ID' },
      { text: '如需异步 Push 通知，在平台「系统变量」设置中开启 push_id 开关' },
      { text: '填写以下配置并保存，自动通过 WebSocket 连接华为 HAG' },
      { text: '在小艺 APP 中发送消息，验证 WS 对话链路' },
    ],
  },
  {
    id: 'weixin',
