...
describe('Knowledge lifecycle E2E (AC-207)', () => {
  ...
  it('import → rebuild → pack-scoped search → delete → no orphans', async () => {
    ...
    db.prepare(`INSERT INTO evidence_passages (doc_anchor, passage_id, content, position, created_at, passage_kind)
      VALUES (?, ?, ?, ?, ?, ?)`).run('dk:test-pack:guide', 'chunk-0', 'Install Node.js v20 or later', 0, now, 'domain_chunk');
    db.prepare(`INSERT INTO evidence_passages (doc_anchor, passage_id, content, position, created_at, passage_kind)
      VALUES (?, ?, ?, ?, ?, ?)`).run('dk:test-pack:guide', 'chunk-1', 'Run pnpm install to set up deps', 1, now, 'domain_chunk');
    ...
    assert.ok(searchResults.some(r => r.anchor === 'dk:test-pack:guide'), 'correct doc returned');
    ...
    const orphanPassages = db.prepare(
      "SELECT count(*) AS c FROM evidence_passages WHERE doc_anchor = 'dk:test-pack:guide'"
    ).get();
  ...
  it('deleteByPackId cleans up all docs and passages for a pack', async () => {
    ...
    const remainingPassages = db.prepare(
      "SELECT count(*) AS c FROM evidence_passages WHERE doc_anchor LIKE 'dk:bulk-pack:%'"
    ).get();
