...
const DOC_KIND_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  ...
  reference: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-300', label: 'Reference' },
  tutorial: { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-800 dark:text-purple-300', label: 'Tutorial' },
...
export function DocKindBadge({ kind }: { kind?: string | null }) {
  ...
  if (!style) return <span className="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-400">{kind}</span>;
  return (
