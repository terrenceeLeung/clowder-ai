    ...
    >
      ...
      >
        ...
        <div className={`overflow-y-auto px-5 py-3 space-y-1 ${showBrowser ? 'hidden' : 'flex-1 min-h-[180px]'}`}>
          ...
          {/* Browsed path not in existing list — show as highlighted entry */}
          {selectedPath &&
            selectedPath !== 'lobby' &&
            ...
            !existingProjects.includes(selectedPath) && (
              <button
                ...
                className="w-full text-left px-3 py-2.5 text-sm text-gray-700 hover:bg-cocreator-bg rounded-lg transition-colors flex items-center gap-2 ring-2 ring-cocreator-primary bg-cocreator-bg"
                title={selectedPath}
              >
                <FolderIcon />
                <div className="min-w-0 flex-1">
                  <span className="font-medium block truncate">{projectDisplayName(selectedPath)}</span>
                  <span className="text-[10px] text-gray-400 block truncate">{selectedPath}</span>
                </div>
