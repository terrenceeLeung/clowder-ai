...
| AC | 描述 |
|----|------|
| AC-A1-1 | `docs/knowledge-map.yaml` 创建，至少 4 个模块（记忆、协作、游戏、基础设施），每个模块定义 name + anchors |
...
- UI 结束按钮 + session 统计
- `/feynman` 命令（输入框快捷入口）
- IndexBuilder 自动检测未归类 anchor → 建议模块
...
### DD-5：knowledge-map.yaml 手工配置

模块划分由 CVO 手工维护，不做自动聚类。

**Why**：模块是产品叙事不是数据分类。Cat Cafe 新 feature ~1-2/周，手动加一行 YAML 不算负担。LLM Wiki 调研也支持用户定义初始 schema 的混合模式。
