...
| AC | 描述 |
|----|------|
| [x] AC-A2-1 | `POST /api/feynman/start` 创建专属 thread，初始化 `feynmanState` 到 thread metadata |
| [x] AC-A2-2 | feynmanState schema：`{ v:1, module, anchors, status:'active'\|'completed', startedAt }` 存在 thread metadata |
| [x] AC-A2-3 | FeynmanPromptSection 实现：检测 feynmanState → 注入三层系统提示词（模块上下文 + 教学协议 + 护栏） |
| [x] AC-A2-4 | SystemPromptBuilder 集成：route 层读取 feynmanState → 传入 InvocationContext → 每次 invocation 动态注入 |
| [x] AC-A2-5 | 模块唯一：同模块再次 start → 返回已有 threadId |
| [x] AC-A2-6 | Explore 模块卡片上的"开始导览"按钮 → 调 start API → 跳转 feynman thread |
| [x] AC-A2-7 | Marker metadata 扩展：callback handler 透传 metadata → Marker interface 加 metadata 字段 → YAML 序列化 |
| [x] AC-A2-8 | 费曼导览能产出结构化 gap：retain-memory 提交含 `{ feynman_type, module, replay_question, evidence_anchors }` |
| [x] AC-A2-9 | 猫讲完或用户说"差不多了"时输出 Delta Report（覆盖 anchors、gaps、Feed candidates、下次复查建议） |
| [x] AC-A2-10 | 抗 sycophancy：用户质疑 validated evidence → 猫重检索后站住；用户新信息 → 标为 correction candidate |

| 2026-04-23 | Phase A-2 merged (PR #7) |
