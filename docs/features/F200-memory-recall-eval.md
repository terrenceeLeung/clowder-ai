...
|----------|---------|------------------------|----------|----------------------|
| recall_quality:low_mrr | ConsumedMRR < 0.20 | totalEvents ≥ 10 AND consumedCount ≥ 5 | high if < 0.10 else medium | F200/ranking |
...
> **minSamples 由来**（codex PR #40 P1 review 2026-06-08）：rate-based 阈值在 `totalEvents=1` 且 no consumption 的低样本窗口会误触发 high `low_mrr`/`high_abandon` finding——这是 low-confidence/no-volume 窗口，不是真实退化信号。`minSamples` precondition 在 trigger 前先 gate 掉低样本误报，落到 `noFindingRecord: { reason: "all_metrics_within_threshold" }` 路径（语义上 = "有数据但不足以判断 rate quality"）。count-based 规则（orphan/stale/verification）不需要 minSamples，因为绝对计数本身是 ground truth。
